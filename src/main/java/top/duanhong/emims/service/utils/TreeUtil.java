...
public class TreeUtil {
     ...
     * 所有待用"菜单"
     */
    private static List<Node> all = null;
     ...
     * @return 转换后的树结构菜单
     */
    public static List<Node> toTree(List<Node> list) {
        // 最初, 所有的 "菜单" 都是待用的
        ...
        for (Node node : roots) {
            node.setChildren(getCurrentNodeChildren(node));
        }
        return roots;
    }
     ...
     * @return 子节点
     */
    private static List<Node> getCurrentNodeChildren(Node parent) {
        // 判断当前节点有没有子节点, 没有则创建一个空长度的 List, 有就使用之前已有的所有子节点.
        List<Node> childList = parent.getChildren() == null ? new ArrayList<>() : parent.getChildren();
